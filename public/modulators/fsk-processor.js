// Frequency Shift Keying Audio Node Processor (Mono Channel)

const SAMPLING_RATE = 48_000;
const TWO_PI = 2 * Math.PI;
const D_TIME = 1 / SAMPLING_RATE;

class FrequencyShiftKeyingProcessor extends AudioWorkletProcessor {
    constructor(options) {
        super(options);
    ...
    process(_inputs, _outputs) {
        ...
        for (let i = 0; i < channel.length; i++) {
            channel[i] = this.getNextSinWaveSample(1, this.carrierFrequencies[this.bit]);
...
registerProcessor('fsk-processor', FrequencyShiftKeyingProcessor);