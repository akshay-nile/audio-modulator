// Frequecny Ramp Modulation Audio Node Processor (Mono Channel)

const SAMPLING_RATE = 48_000;
const TWO_PI = 2 * Math.PI;
const D_TIME = 1 / SAMPLING_RATE;

class FrequecnyRampModulationProcessor extends AudioWorkletProcessor {
    constructor(options) {
        super(options);
        ...
        this.minCarrierFrequency = carrierFreq - baudRate;
        this.maxCarrierFrequency = carrierFreq + baudRate;

        this.sampleCounter = 0;
        this.samplesPerBit = Math.floor(SAMPLING_RATE / options.processorOptions.baudRate);
        this.stepPerSample = 2 * baudRate / (this.samplesPerBit - 1);

        this.port.onmessage = e => {
            if (typeof e.data === 'object' && 'baudRate' in e.data && 'carrierFreq' in e.data) {
                const { carrierFreq, baudRate } = e.data;
                this.minCarrierFrequency = carrierFreq - 5 * baudRate;
                this.maxCarrierFrequency = carrierFreq + 5 * baudRate;
                this.samplesPerBit = Math.floor(SAMPLING_RATE / baudRate);
                this.stepPerSample = 10 * baudRate / (this.samplesPerBit - 1);
    ...
    process(_inputs, _outputs) {
        ...
        for (let i = 0; i < channel.length; i++) {
            const step = this.stepPerSample * this.sampleCounter;
            const frequency = this.bit ? this.minCarrierFrequency + step : this.maxCarrierFrequency - step;

            channel[i] = this.getNextSinWaveSample(1, frequency);
...
registerProcessor('frm-processor', FrequecnyRampModulationProcessor);