// Amplitude Ramp Modulation Audio Node Processor (Mono Channel)

const SAMPLING_RATE = 48_000;
const TWO_PI = 2 * Math.PI;
const D_TIME = 1 / SAMPLING_RATE;

class AmplitudeRampModulationProcessor extends AudioWorkletProcessor {
    constructor(options) {
        super(options);
        ...
        this.stepPerSample = 1 / (this.samplesPerBit - 1);
    ...
    process(_inputs, _outputs) {
        ...
        for (let i = 0; i < channel.length; i++) {
            let amplitude = this.stepPerSample * this.sampleCounter;
            if (this.bit === 0) amplitude = 1 - amplitude;

            channel[i] = this.getNextSinWaveSample(amplitude, this.carrierFrequency);
...
registerProcessor('arm-processor', AmplitudeRampModulationProcessor);